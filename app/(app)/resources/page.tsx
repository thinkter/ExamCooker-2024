...
type ResourcesSearchParams = Promise<{ page?: string; search?: string }>;
type ResolvedResourcesSearchParams = Awaited<ResourcesSearchParams>;
...
async function ResourcesResults({
  params,
}: {
  ...
  if (validatedPage !== page) {
    redirect(
      `/resources?page=${validatedPage}${search ? `&search=${encodeURIComponent(search)}` : ""}`,
    );
  }
...
export default async function ResourcesPage({
  ...
  const resolvedSearchParams: ResolvedResourcesSearchParams =
    (await searchParams) ?? {};
    ...
    <div className="transition-colors min-h-screen text-black dark:text-gray-200">
      <div className="container mx-auto px-2 sm:px-4 lg:px-8 py-2 sm:py-4 lg:py-8">
        ...
        <Suspense fallback={<ResourcesSkeleton />}>
          <ResourcesResults params={resolvedSearchParams} />
        </Suspense>
      </div>
    </div>
  );
...
export async function generateMetadata({
  ...
  return {
    title,
    description:
      "Browse course resources and reference material on ExamCooker.",
