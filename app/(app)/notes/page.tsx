...
type ResolvedNotesSearchParams = Awaited<NotesSearchParams>;
...
async function NotesResults({
  searchParams,
}: {
  searchParams: ResolvedNotesSearchParams;
}) {
  const pageSize = 9;
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-5 p-2 sm:p-4 lg:p-6 place-content-center">
          {paginatedNotes.length > 0 ? (
            paginatedNotes.map((eachNote, index) => (
              <NotesCard key={eachNote.id} note={eachNote} index={index} />
            ))
          ) : (
...
export default async function NotesPage({
  ...
  const resolvedSearchParams: ResolvedNotesSearchParams =
    (await searchParams) ?? {};
  ...
  return (
    <div className="p-2 sm:p-4 lg:p-8 transition-colors flex flex-col min-h-screen items-center text-black dark:text-[#D5D5D5]">
      ...
      <Suspense fallback={<NotesSkeleton />}>
        <NotesResults searchParams={resolvedSearchParams} />
      </Suspense>
    </div>
  );
}
