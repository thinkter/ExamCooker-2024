...
type NotePageParams = Promise<{ id: string }>;
type ResolvedNotePageParams = Awaited<NotePageParams>;
...
async function PdfViewerPage({ params }: { params: NotePageParams }) {
  let year: string = "";
  let slot: string = "";
  let note;
  const { id }: ResolvedNotePageParams = await params;
    ...
    return (
      <div>
        <div className="text-center p-8 dark:text-[#d5d5d5]">
          Error loading note. Please refresh, or try again later.
        </div>
  ...
  const postTime: string = note.createdAt.toLocaleString("en-US", {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen text-black dark:text-[#D5D5D5]">
      ...
      />
      <ViewTracker id={note.id} type="note" title={title} />
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <div className="space-y-2 sm:space-y-3">
              ...
              <p className="text-base sm:text-lg">
                ...
                {note.author?.name?.slice(0, -10) || "Unknown"}
              </p>
              {note.tags?.length ? (
                <div className="pt-2">
