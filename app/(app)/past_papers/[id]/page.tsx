...
type PastPaperPageParams = Promise<{ id: string }>;
type ResolvedPastPaperPageParams = Awaited<PastPaperPageParams>;
...
async function PdfViewerPage({ params }: { params: PastPaperPageParams }) {
  let paper;
  let year: string = "";
  let slot: string = "";
  const { id }: ResolvedPastPaperPageParams = await params;
    ...
    return (
      <div>
        <div className="text-center p-8 dark:text-[#d5d5d5]">
          Error loading paper. Please refresh, or try again later.
        </div>
  ...
  const isModerator =
    (session?.user as { role?: string } | undefined)?.role === "MODERATOR";

  const postTime: string = paper.createdAt.toLocaleString("en-US", {
    timeZone: "Asia/Kolkata",
  });

  const parsedTitle = parsePaperTitle(paper.title);
  const courseTags = paper.tags.filter((tag) => extractCourseFromTag(tag.name));
  const courseFromTag = courseTags.length
    ? extractCourseFromTag(courseTags[0].name)
    : null;
  const parsedCourseCode = parsedTitle.courseCode
    ?.replace(/\s+/g, "")
    .toUpperCase();
  const taggedCourseCode = courseFromTag?.code
    ?.replace(/\s+/g, "")
    .toUpperCase();
  const useParsedCourse = Boolean(
    parsedCourseCode &&
      (!taggedCourseCode || parsedCourseCode !== taggedCourseCode),
  );
  ...
  const courseCode = useParsedCourse
    ? parsedCourseCode
    : (courseFromTag?.code ?? parsedCourseCode);
  const displayTitle =
    courseCode && !courseTitle.toUpperCase().includes(courseCode)
  ...
  const jsonLd = {
    ...
    author: paper.author?.name
      ? { "@type": "Person", name: paper.author.name }
      : undefined,
  };
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {relatedPapers.map((item, index) => (
          <PastPaperCard key={item.id} pastPaper={item} index={index} />
        ))}
      </div>
    </div>
  ) : null;
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <div className="space-y-2 sm:space-y-3">
              <p className="text-base sm:text-lg">
                <span className="font-semibold">Slot:</span> {displaySlot}
              </p>
              <p className="text-base sm:text-lg">
              ...
              <div className="flex gap-2 items-center justify-between">
                <p className="text-base sm:text-xs">
                  <span className="font-semibold">
                    Posted at: {TimeHandler(postTime).hours}:
                    {TimeHandler(postTime).minutes}
                    {TimeHandler(postTime).amOrPm}, {TimeHandler(postTime).day}-
                    {TimeHandler(postTime).month}-{TimeHandler(postTime).year}
                  </span>
                </p>
                <ItemActions
                  itemId={paper.id}
