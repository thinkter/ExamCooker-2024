...
type PastPapersSearchParams = Promise<{
  page?: string;
  search?: string;
  tags?: string | string[];
}>;
type ResolvedPastPapersSearchParams = Awaited<PastPapersSearchParams>;
...
async function PastPaperResults({
  ...
  searchParams: ResolvedPastPapersSearchParams;
}) {
  const pageSize = 9;
...
export default async function PastPaperPage({
  ...
  searchParams?: PastPapersSearchParams;
}) {
  const resolvedSearchParams: ResolvedPastPapersSearchParams =
    (await searchParams) ?? {};
  ...
  return (
    <div className="p-2 sm:p-4 lg:p-8 transition-colors flex flex-col min-h-screen items-center text-black dark:text-[#D5D5D5] w-full overflow-x-hidden">
      ...
      <Suspense fallback={<PastPapersSkeleton />}>
        <PastPaperResults searchParams={resolvedSearchParams} />
      </Suspense>
    </div>
  );
}
...
export async function generateMetadata({
  ...
  const tags: string[] = Array.isArray(resolvedSearchParams.tags)
